...
//    // 삭제
//    // 주소 설계: http://localhost:8080/board/10/delete (form 활용이기 때문에 delete 선언)
...
//    }
    ...
    public String delete(@PathVariable (name = "id") Integer id){
        // 유효성, 인증검사
        // 세션에서 로그인 사용자 정보 가져오기 -> 인증(로그인 여부), 인가(권한)
        User sessionUser = (User) session.getAttribute("sessionUser");
    ...
    // 게시글 수정 화면 요청
    // 주소 설계: http://localhost:8080/board/id/update
    ...
    public String updateForm(@PathVariable(name = "id") Integer id, HttpServletRequest request){
        ...
        request.setAttribute("board", board);
        // 뷰 리졸브 - 템플릿 반환
        return  "board/update";    // src/main/resources/templates/board/update-form.mustache
    }

    // 게시글 수정 요청 기능
    // 주소 설계: http://localhost:8080/board/{id}/update
    @PostMapping("/board/{id}/update")
    public String update(@PathVariable(name = "id") Integer id, @ModelAttribute BoardDTO.UpdateDTO reqUpdateDTO){
        // 1. 데이터 바인딩 방식 수정
        ...
        boardRepository.updateByIdJPA(id, reqUpdateDTO.getTitle(), reqUpdateDTO.getContent());
